 ...
 *	_options_pagename - 'tm_dashboard_options_pagename',
 *
 *	plugin_class_name - 'tm_dashboard_plugin_class_name',
 *
 *	save_button_text - 'tm_dashboard_save_button_text',
 *
 *	plugin_name - 'tm_dashboard_plugin_name',
 *
...
define( 'TM_DASHBOARD_DIR_V2', trailingslashit( dirname(__FILE__) ) );
define( 'TM_DASHBOARD_PLUGIN_URI_V2', plugins_url( '', __FILE__ ) );

class Tm_Dashboard_v2 {


	function __construct( $args ) {
		//define filterable variables
		$this->_options_pagename = isset( $args['tm_dashboard_options_pagename'] ) ? $args['tm_dashboard_options_pagename'] : 'tm_dashboard';
		$this->plugin_class_name = isset( $args['tm_dashboard_plugin_class_name'] ) ? $args['tm_dashboard_plugin_class_name'] : '';
		$this->save_button_text = isset( $args['tm_dashboard_save_button_text'] ) ? $args['tm_dashboard_save_button_text'] : esc_html__( 'Save Changes', 'tm_dashboard' );
		$this->plugin_name = isset( $args['tm_dashboard_plugin_name'] ) ? $args['tm_dashboard_plugin_name'] : 'tm_dashboard';
		$this->options_path = isset( $args['tm_dashboard_options_path'] ) ? $args['tm_dashboard_options_path'] : TM_DASHBOARD_DIR_V2 . 'includes/options.php';
		$this->top_level_page = isset( $args['tm_dashboard_options_page'] ) ? $args['tm_dashboard_options_page'] : 'tools';
	...
	function add_class_localization() {
		load_plugin_textdomain( 'tm_dashboard', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
	}

	function get_options_array() {
		return get_option( 'tm_' . $this->plugin_name . '_options' ) ? get_option( 'tm_' . $this->plugin_name . '_options' ) : array();
	}

	public static function load_fonts_class() {
		if ( ! class_exists( 'TM_Dashboard_Fonts' ) ) {
			require_once( TM_DASHBOARD_DIR_V2 . 'includes/google-fonts.php' );
		}

		$fonts_class = new Tm_Dashboard_Fonts();
	...
	function update_option( $update_array ) {
		...
		update_option( 'tm_' . $this->plugin_name . '_options', $updated_options );
	}
	...
	function remove_option( $option_key ) {
		...
		if ( isset( $dashboard_options[$option_key] ) ) {
			...
			update_option( 'tm_' . $this->plugin_name . '_options', $dashboard_options );
		}
	}
	...
	function dashboard_scripts( $hook ) {
		...
		wp_enqueue_script( 'tm-dashboard-mce-js', TM_DASHBOARD_PLUGIN_URI_V2 . '/js/tinymce/js/tinymce/tinymce.min.js', array( 'jquery' ), $this->class_version, true );
		wp_enqueue_style( 'tm-dashboard-css', TM_DASHBOARD_PLUGIN_URI_V2 . '/css/tm-dashboard.css', array(), $this->class_version );
		wp_enqueue_script( 'tm-dashboard-js', TM_DASHBOARD_PLUGIN_URI_V2 . '/js/tm-dashboard.js', array( 'jquery' ), $this->class_version, true );
	...
	function api_is_network_authorized( $network ) {
		$is_authorized = apply_filters( 'tm_' . $this->plugin_name . '_authorization_verdict', false, $network );
	...
	function process_and_update_options( $options, $option_sub_title = '' ) {
					...
					foreach( $value[ 'contents' ] as $key => $value ) {
						...
						if ( isset( $options_array ) ) {
							foreach( $options_array as $option ) {
								...
								do_action( 'tm_' . $this->plugin_name . '_after_save_options', $dashboard_options_temp, $current_option_name, $option, $output );
		...
		} TM_Dashboard_v2::update_option( $final_array );
	 ...
	 *	'tm_<plugin_name>_after_main_options'
	 *	'tm_<plugin_name>_after_header_options'
	 *	'tm_<plugin_name>_after_save_button'
	 *	'tm_<plugin_name>_header_start'
	 *	'tm_<plugin_name>_header_end'
	 *
	 * @return array
	 */
	function generate_options_page( $sub_array = '' ) {
		...
		printf(
			'<div id="tm_dashboard_wrapper_outer" class="%1$s">
				<div id="tm_dashboard_wrapper" class="tm_dashboard">
					<div id="tm_dashboard_header">
						<div id="tm_dashboard_logo" class="tm_dashboard_icon_%2$s tm_dashboard_icon"></div>
						<ul>',
		...
		if ( isset( $dashboard_sections[ 'header' ][ 'contents' ] ) ) {
			foreach ( $dashboard_sections[ 'header' ][ 'contents' ] as $key => $value ) {
				printf(
					'<li class="tm_dashboard_tab_content_header_%1$s">
						<a href="#tab_tm_dashboard_tab_content_header_%1$s" id="tm_dashboard_tab_content_header_%1$s" class="tm_dashboard_icon_header_%1$s tm_dashboard_icon">
							<span></span>
						</a>
					</li>',
					esc_attr( $key )
		...
		echo '
					...
					<div id="tm_dashboard_navigation">
						<ul>';
			...
			foreach ( $dashboard_sections as $key => $value ) {
				if ( $key !== 'header') {
					...
					foreach( $value as $key => $value ) {
						if ( $key == 'title' ){
							printf(
								'<li class="tm_dashboard_tab_content_side_%1$s">
									<a href="#" class="tm_dashboard_icon_%1$s tm_dashboard_icon tm_dashboard_tab_parent">
										<span>%2$s</span>
									</a>',
							...
							printf( '<ul class="tm_dashboard_%1$s_nav">',
								esc_attr( $current_section )
							);
							foreach( $value as $key => $value ) {
								printf(
									'<li class="tm_dashboard_tab_content_side_%2$s">
										<a href="#tab_tm_dashboard_tab_content_%1$s_%2$s" id="tm_dashboard_tab_content_%1$s_%2$s" class="tm_dashboard_icon_%2$s tm_dashboard_icon">
											<span>%3$s</span>
										</a>
									</li>',
									esc_attr( $current_section ),
		...
		echo '
				...
				<div id="tm_dashboard_content">
					<form id="tm_dashboard_options" enctype="multipart/form-data">';
		settings_fields( 'tm_dashboard_settings_group' );
			...
			foreach ( $dashboard_sections as $key => $value ) {
				...
				if ( $key !== 'header' ) {
					foreach( $value[ 'contents' ] as $key => $value ) {
						...
						printf(
							'<div class="tm_dashboard_tab_content tm_dashboard_tab_content_%1$s_%2$s">',
							esc_attr( $current_section ),
							esc_attr( $key )
						);
						foreach( $options_array as $option) {
							...
							switch( $option[ 'type' ] ) {
								case 'select_shape' :
									printf(
										'<div class="tm_dashboard_row tm_dashboard_selection%2$s%4$s"%3$s%5$s>
											<h2>%1$s</h2>
											<div style="clear:both;"></div>',
										esc_html( $option[ 'title' ] ),
										isset( $option[ 'display_if' ] ) ? ' tm_dashboard_hidden_option' : '',
									...
									foreach ( $option[ 'value' ] as $shape ){
										printf(
											'<div class="tm_dashboard_shape tm_dashboard_icon tm_dashboard_single_selectable %1$s">
												<div class="tm_dashboard_shape_tile tm_dashboard_icon tm_dashboard_shape_%2$s"></div>
												<input type="radio" class="tm_dashboard[%3$s]" name="tm_dashboard[%3$s]" value="%2$s" %4$s style="position: absolute; z-index: -1; visibility: hidden;">
											</div>',
											$shape === $current_option_value ? 'tm_dashboard_selected' : '',
											esc_attr( $shape ),
								...
								case 'select' :
									...
									printf(
										'<li class="select%3$s%5$s%7$s"%4$s%6$s%8$s>
											<p>%1$s</p>
											<select name="tm_dashboard[%2$s]">',
										isset( $option['title_' . $current_location] ) ? esc_html( $option['title_' . $current_location] ) : esc_html( $option['title'] ),
										esc_attr( $current_option_name ),
										isset( $option[ 'display_if' ] ) ? ' tm_dashboard_hidden_option tm_dashboard_triggered_option' : '',
										isset( $option[ 'display_if' ] ) ? ' data-condition="' . esc_attr( $option[ 'display_if' ] ) .  '"': '',
										isset( $option[ 'conditional' ] ) ? ' tm_dashboard_conditional' : '',
								...
								case 'checkbox' :
									printf( '
										<li class="tm_dashboard_checkbox clearfix%5$s%6$s%9$s"%4$s%7$s%8$s>
											<p>%1$s</p>
											<input type="checkbox" id="tm_dashboard[%2$s]" name="tm_dashboard[%2$s]" value="1" %3$s>
											<label for="tm_dashboard[%2$s]"></label>',
										isset( $option['title_' . $current_location] ) ? esc_html( $option['title_' . $current_location] ) : esc_html( $option['title'] ),
										esc_attr( $current_option_name ),
								...
								case 'checkbox_set' :
									...
									if ( ! empty( $checkboxes_array ) ) {
										...
										foreach ( $checkboxes_array as $value => $label ) {
											printf( '
												<li class="tm_dashboard_checkbox%6$s%8$s"%7$s>
													<input type="checkbox" id="tm_dashboard[%1$s][%4$s]" name="tm_dashboard[%1$s][]" value="%3$s" %2$s>
													<label for="tm_dashboard[%1$s][%4$s]"></label>
													<p>%5$s</p>
												</li>',
												esc_attr( $current_option_name ),
												...
												esc_attr( $i ),
												esc_attr( $label ), //#5
												isset( $option[ 'conditional' ][$value] ) ? ' tm_dashboard_conditional' : '',
								...
								case 'input_field' :
									printf(
										'<li class="input clearfix%4$s%7$s"%5$s%10$s>
											<p>%1$s</p>
											<input type="%9$s" name="tm_dashboard[%2$s]" value="%3$s" placeholder="%6$s"%8$s>',
										...
										'text' == $option[ 'subtype' ] ? ' tm_dashboard_longinput' : '',
										( isset( $option['class'] )
								...
								case 'checkbox_posts' :
									...
									foreach ( $post_types as $post_type => $id ){
										...
										if ( 'post_types' === $option['subtype'] ) {
											if ( isset( $option[ 'conditional' ]['any_post'] ) && 'page' !== $id ) {
												$conditional_class = ' tm_dashboard_conditional';
												$conditional_data = sprintf( ' data-enables="%1$s"', esc_attr( $option['conditional']['any_post'] ) );
											}

											$conditional_class = isset( $option['conditional'][$id] ) ? ' tm_dashboard_conditional' : $conditional_class;
										...
										printf( '
											<li class="tm_dashboard_checkbox%6$s"%7$s>
												<input type="checkbox" id="tm_dashboard[%1$s][%4$s]" name="tm_dashboard[%1$s][]" value="%3$s" %2$s>
												<label for="tm_dashboard[%1$s][%4$s]"></label>
												<p>%5$s</p>
											</li>',
											...
											( 'post_cats' === $option['subtype'] && isset( $option['include_custom'] ) )
												? esc_attr( $id ) . esc_html__( ' ( post )', 'tm_dashboard' )
												: esc_attr( $id ),
									...
									if ( 'post_cats' === $option['subtype'] ) {
										...
										printf( '
											<li class="tm_dashboard_checkbox tm_dashboard_auto_select">
												<input type="checkbox" id="tm_dashboard[%1$s][auto_select]" name="tm_dashboard[%1$s][auto_select]" value="1" %2$s>
												<label for="tm_dashboard[%1$s][auto_select]"></label>
												<p>%3$s</p>
											</li>',
											esc_attr( $current_option_name ),
											checked( $current_option_value['auto_select'], 1, false ),
											esc_html__( 'Automatically check categories created in future', 'tm_dashboard' )
										);
										...
										printf( '
											<li>
												<input type="hidden" id="tm_dashboard[%1$s][previously_saved]" name="tm_dashboard[%1$s][previously_saved]" value="%2$s" />
											</li>',
											esc_attr( $current_option_name ),
								...
								case 'section_start' :
									printf(
										'%5$s<div class="tm_dashboard_form tm_dashboard_row%2$s%7$s"%3$s%4$s%8$s>
											%1$s
											%6$s
											<div style="clear:both;"></div>
											<ul>',
										...
										( isset( $current_option_name ) && '' != $current_option_name )
											? sprintf( ' data-name="tm_dashboard[%1$s]"', esc_attr( $current_option_name ) )
											: '',
										( isset( $option[ 'sub_section' ] ) && true == $option[ 'sub_section' ] )
											? '<li class="tm_dashboard_auto_height">'
											: '', //#5
										isset( $option[ 'subtitle' ] )
											? sprintf('<p class="tm_dashboard_section_subtitle">%1$s</p>', esc_html( $option[ 'subtitle' ] ) )
											: '',
										isset( $option[ 'class' ] ) ? ' ' . esc_attr( $option[ 'class' ] ) : '',
										isset( $option[ 'display_if' ] ) ? ' data-triggers_count="0"': '' //#8
									);
								...
								case 'text' :
									printf(
										'<li class="tm_dashboard_auto_height%6$s%10$s"%7$s%8$s>
											%9$s
											<textarea placeholder="%1$s" rows="%2$s" id="tm_dashboard_%4$s" name="tm_dashboard[%4$s]"%5$s>%3$s</textarea>
										</li>',
										...
										! empty( $option[ 'title' ] ) ? ' tm_dashboard_text_with_title' : '' //#10
									);
								break;

								case 'main_title' :
									printf(
										'<div class="tm_dashboard_row tm_dashboard_selection%3$s">
											<h1>%1$s</h1>
											%2$s
										</div>',
										esc_html( $option[ 'title' ] ),
								...
								case 'note' :
									printf(
										'<div class="tm_dashboard_row tm_dashboard_note">
											<h2>%1$s</h2>
											<p>
												<span>%2$s</span>
											</p>
										</div>',
										esc_html__( 'Note:', 'tm_dashboard' ),
								...
								case 'color_picker' :
									printf(
										'<li class="input clearfix tm_dashboard_color_picker%5$s%8$s"%6$s%7$s>
											<p>%4$s</p>
											<input class="tm-dashboard-color-picker" type="text" maxlength="7" placeholder="%1$s" name=tm_dashboard[%2$s] value="%3$s" />
										</li>',
										esc_attr( $option[ 'placeholder' ] ),
										esc_attr( $current_option_name ),
										esc_attr( $current_option_value ),
										esc_html( $option[ 'title' ] ),
										isset( $option[ 'display_if' ] ) ? ' tm_dashboard_hidden_option' : '', // #5
										isset( $option[ 'display_if' ] ) ? ' data-condition="' . esc_attr( $option[ 'display_if' ] ) .  '"': '',
										isset( $option[ 'display_if' ] ) ? ' data-triggers_count="0"' : '',
								...
								case 'live_search' :
										...
										if ( ! empty( $selected_posts ) ) {
											foreach( $selected_posts as $single_post ) {
												$selected_posts_list .= sprintf( '
													<span data-post_id="%3$s">[%1$s] -  %2$s<span class="tm_dashboard_menu_remove"></span></span>',
													esc_html( $single_post->post_type ),
									...
									printf( '
										<li class="tm_dashboard_selected">%1$s</li>
										<li class="tm_dashboard_live_fields">
											<input type="text" class="tm_dashboard_search_posts" placeholder="%4$s" data-post_type="%5$s"/><span class="spinner"></span>
											<input type="hidden" id="tm_dashboard[%2$s]" name="tm_dashboard[%2$s]" value="%3$s" />
										</li>
										<li class="tm_dashboard_live_search_res">
											<ul class="tm_dashboard_search_results"></ul>
										</li>',
										$selected_posts_list,
										esc_attr( $current_option_name ),
								...
								case 'image_upload' :
									printf( '
										<li class="tm_dashboard_upload_image%7$s"%8$s%9$s>
											<p>%6$s</p>
											<input name="tm_dashboard[%1$s][url]" type="text" class="tm-dashboard-upload-field" value="%2$s" />
											<input type="hidden" class="tm-dashboard-upload-id" name="tm_dashboard[%1$s][id]" value="%10$s">
											<input type="button" class="button button-upload tm-dashboard-upload-button" value="%3$s" data-choose="%4$s" data-update="%5$s" data-type="image" />
										</li>',
										esc_attr( $current_option_name ),
								...
								case 'hidden_option' :
									if ( 'array' == $option[ 'subtype' ] ) {
										...
										foreach ( $current_option_value as $single_value ) {
											printf( '<input name="tm_dashboard[%1$s][]" type="hidden" value="%2$s" />',
												esc_attr( $current_option_name ),
												esc_attr( $single_value )
											);
										}
									} else {
										printf( '<input name="tm_dashboard[%1$s]" id="tm_dashboard_%1$s" type="hidden" value="%2$s" />',
											esc_attr( $current_option_name ),
											esc_attr( $current_option_value )
										);
									}
								...
								case 'button' :
									printf(
										'<li class="tm_dashboard_action_button">
											<a href="%1$s" class="tm_dashboard_icon %2$s">%3$s</a>
											<span class="spinner"></span>
										</li>',
										esc_url( $option[ 'link' ] ),
										esc_html( $option[ 'class' ] ),
										( true == $option[ 'authorize' ] && $this->api_is_network_authorized( $option[ 'action' ] ) )
											? esc_html__( 'Re-Authorize', 'tm_dashboard' ) :
											esc_html( $option[ 'title' ] )
									);
								break;

								case 'font_select' :
									$fonts_class = TM_Dashboard_v2::load_fonts_class();

									$current_option_list = $fonts_class->tm_get_google_fonts();
									...
									printf(
										...
										isset( $option[ 'display_if' ] )
											? ' tm_dashboard_hidden_option tm_dashboard_triggered_option'
											: '',
										isset( $option[ 'display_if' ] ) ? ' data-condition="' . esc_attr( $option[ 'display_if' ] ) .  '"': '',
										isset( $option[ 'conditional' ] ) ? ' tm_dashboard_conditional' : '',
										( isset( $option[ 'conditional' ] )
									...
									foreach ( $current_option_list as $font_name => $font_details ) {
										printf( '<option value="%1$s" class="tm_dashboard_font_%1$s" %2$s>%3$s</option>',
											esc_attr( $font_name ),
							...
							do_action( 'tm_' . $this->plugin_name . '_after_main_options', $option, $current_option_value );
						} // end foreach( $options_array as $option)
		...
		printf(
			'<div class="tm_dashboard_row tm_dashboard_save_changes %3$s">
				<button class="tm_dashboard_icon"%2$s>%1$s</button>
				<span class="spinner"></span>
			</div>
			...
			'' !== $sub_array
				...
				: '',
	 		apply_filters( 'tm_' . $this->plugin_name . '_save_button_class', '' )
		);

		do_action( 'tm_' . $this->plugin_name . '_after_save_button' );

		echo '</form>';

		if ( isset( $dashboard_sections[ 'header' ][ 'contents' ] ) ) {
			do_action( 'tm_' . $this->plugin_name . '_header_start' );

			foreach ( $dashboard_sections[ 'header' ][ 'contents' ] as $key => $value ) {
				$options_array = $dashboard_options_assigned['header_' . $key . '_options'];

				printf(
					'<div class="tm_dashboard_tab_content tm_dashboard_tab_content_header_%1$s tm_dashboard_header_option">',
					esc_attr( $key )
				);
				if ( isset( $options_array ) ) {
					foreach( $options_array as $option ) {
						switch( $option[ 'type' ] ) {
							case 'import_export' :
								printf(
									'<div class="tm_dashboard_form tm_dashboard_row">
										<h1>%1$s</h1>
										<p>%2$s</p>
									</div>
									<div class="tm_dashboard_import_form tm_dashboard_row">
										<h2>%3$s</h2>
										<p class="tm_dashboard_section_subtitle">%4$s</p>
										<form method="post">
											<input type="hidden" name="tm_dashboard_action" value="export_settings" />
											<p>',
									esc_html( $option[ 'title' ] ),
									sprintf( esc_html__( 'You can either export your %1$s Settings or import settings from another install of %1$s below.', 'tm_dashboard' ), esc_html( ucfirst( $this->plugin_name ) ) ),
									sprintf( esc_html__( 'Export %1$s Settings', 'tm_dashboard' ), esc_html( ucfirst( $this->plugin_name ) ) ),
									esc_html__( 'Export the plugin settings for this site as a .json file. This allows you to easily import the configuration into another site.', 'tm_dashboard' )
								);

								wp_nonce_field( 'tm_dashboard_export_nonce', 'tm_dashboard_export_nonce' );

								printf(
									'			<button class="tm_dashboard_icon tm_dashboard_icon_importexport" type="submit" name="submit_export" id="submit_export">' . esc_html__( 'Export', 'tm_dashboard' ) . '</button>
											</p>
										</form>
									</div>
									...
									<div class="tm_dashboard_form tm_dashboard_row">
										<h2>%1$s</h2>
										<div class="tm_dashboard_import_form tm_dashboard_row">
											<p class="tm_dashboard_section_subtitle">%2$s</p>
											<form method="post" enctype="multipart/form-data" action="%4$s.php?page=%3$s#tab_tm_dashboard_tab_content_header_importexport">
												<input type="file" name="import_file"/>',
									sprintf( esc_html__( 'Import %1$s Settings', 'tm_dashboard' ), esc_html( ucfirst( $this->plugin_name ) ) ),
									esc_html__( 'Import the plugin settings from a .json file. This file can be obtained by exporting the settings on another site using the form above.', 'tm_dashboard' ),
									$this->_options_pagename,
									'toplevel_page' == $this->top_level_page ? 'admin' : $this->top_level_page
								);

								wp_nonce_field( 'tm_dashboard_import_nonce', 'tm_dashboard_import_nonce' );

								echo '
											<button class="tm_dashboard_icon tm_dashboard_icon_importexport" type="submit" name="submit_import" id="submit_import">' . esc_html__( 'Import', 'tm_dashboard' ) . '</button>
											<input type="hidden" name="tm_dashboard_action" value="import_settings" />
										</form>
									</div>
								</div>';
						...
						do_action( 'tm_' . $this->plugin_name . '_after_header_options', $option, $dashboard_options );
					} // end foreach( $options_array as $option )
				} // end if ( isset( $options_array ) )

				echo '</div><!-- .tm_dashboard_tab_content_header_ -->';
			} // end foreach ( $dashboard_sections[ 'header' ][ 'contents' ] as $key => $value )

			do_action( 'tm_' . $this->plugin_name . '_header_end' );
	...
	function remove_site_specific_fields( $settings ) {
		$remove_options = apply_filters( 'tm_' . $this->plugin_name . '_export_exclude', array(
			'access_tokens',
			'db_version',
		) );
	...
	function process_settings_import() {
		...
		$sub_array = apply_filters( 'tm_' . $this->plugin_name . '_import_sub_array', false );

		$error_message = $this->prepare_import_settings( apply_filters( 'tm_' . $this->plugin_name . '_import_array', $dashboard_settings ), $sub_array );
