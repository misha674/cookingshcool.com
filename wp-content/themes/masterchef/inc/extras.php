 ...
 * @return string
 */
function masterchef_set_post_meta_value( $value ) {
	$queried_obj = apply_filters( 'masterchef_queried_object_id', false );
	...
	$curr_opions = 'masterchef_' . str_replace( 'theme_mod_', '', current_filter() );
 ...
 * Sidebar position
 */
 add_filter( 'theme_mod_sidebar_position', 'masterchef_set_post_meta_value' );
 
 /**
 * Header container type
 */
 add_filter( 'theme_mod_header_container_type', 'masterchef_set_post_meta_value' );
 
 /**
 * Content container type
 */
 add_filter( 'theme_mod_content_container_type', 'masterchef_set_post_meta_value' );
 
 /**
  * Footer container type
  */
 add_filter( 'theme_mod_fotter_container_type', 'masterchef_set_post_meta_value' );
 ...
 * @return string
 */
function masterchef_render_macros( $string ) {

	$macros = apply_filters( 'masterchef_data_macros', array(
		'/%%year%%/' => date( 'Y' ),
 ...
 * @return string
 */
function masterchef_render_icons( $content ) {
	$icons     = masterchef_get_render_icons_set();
	$icons_set = implode( '|', array_keys( $icons ) );

	$regex = '/icon:(' . $icons_set . ')?:?([a-zA-Z0-9-_]+)/';

	return preg_replace_callback( $regex, 'masterchef_render_icons_callback', $content );
}
 ...
 * @return string
 */
function masterchef_render_icons_callback( $matches ) {
 ...
 * @return array
 */
function masterchef_get_render_icons_set() {
	return apply_filters( 'masterchef_render_icons_set', array(
 ...
 * @return string
 */
function masterchef_render_theme_url( $url ) {
 ...
 */
function masterchef_get_image_id_by_url( $image_src ) {
	global $wpdb;
...
function masterchef_post_formats_gallery() {
	$size = masterchef_post_thumbnail_size();
	...
	$images = masterchef_theme()->get_core()->modules['cherry-post-formats-api']->get_gallery_images(false);
 ...
 * @return bool
 */
function masterchef_is_meta_visible( $meta, $context = 'loop' ) {

	if ( ! $meta ) {
		return false;
	}

	$meta_enabled = get_theme_mod( $meta, masterchef_theme()->customizer->get_default( $meta ) );
 ...
 * @return array
 */
function masterchef_post_thumbnail_size( $args = array() ) {
	$args = wp_parse_args( $args, array(
		'small'        => 'post-thumbnail',
		'fullwidth'    => 'masterchef-thumb-l',
		'class_prefix' => '',
	) );

	$layout      = get_theme_mod( 'blog_layout_type', masterchef_theme()->customizer->get_default( 'blog_layout_type' ) );
	$format      = get_post_format();
	$size_option = get_theme_mod( 'blog_featured_image', masterchef_theme()->customizer->get_default( 'blog_featured_image' ) );
