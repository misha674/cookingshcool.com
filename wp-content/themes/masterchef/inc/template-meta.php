 ...
 * @return array
 */
function masterchef_share_buttons( $context = 'loop', $args = array(), $config = array() ) {
	...
	if ( ! masterchef_is_meta_visible( $meta, $context ) ) {
		return;
	}
	...
	$default_config = apply_filters( 'masterchef_default_config_share_buttons', array(
	...
	$share_item_html = apply_filters( 'masterchef_share_button_html',
	...
	foreach ( (array) $networks as $id => $network ) :
		...
		$share_buttons .= sprintf(
			...
			esc_html__( 'Share on ', 'masterchef' ) . $network['name'],
	...
	$share_label = '';
	if( $meta === 'single_post_share_buttons' ){
		$share_label = get_theme_mod( 'single_post_share_label', masterchef_theme()->customizer->get_default( 'single_post_share_label' ) );
		$share_label = $share_label !== '' ? '<h6 class="share-btns__label">' . $share_label . '</h6>' : '';
	}
