 ...
 * @package Masterchef
 */
if ( ! class_exists( 'Masterchef_Carousel_Widget' ) ) {

	class Masterchef_Carousel_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_description	= esc_html__( 'Display a list of your posts on your site.', 'masterchef' );
			$this->widget_id			= 'masterchef_widget_carousel';
			$this->widget_name			= esc_html__( 'Carousel', 'masterchef' );
			...
			$this->settings				= array(
				...
				'posts_per_page' => array(
					...
					'max_value'			=> 20,
					'min_value'			=> 1,
					'label'				=> esc_html__( 'Posts count', 'masterchef' ),
				),
				'post_title' => array(
					'type'				=> 'switcher',
					'value'				=> 'true',
					'style'				=> ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'				=> esc_html__( 'Display title', 'masterchef' ),
				),
				'content' => array(
					'type'				=> 'switcher',
					'value'				=> 'false',
					'style'				=> ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'				=> esc_html__( 'Display content', 'masterchef' ),
				),
				'more_button' => array(
					'type'				=> 'switcher',
					'value'				=> 'false',
					'style'				=> ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'				=> esc_html__( 'Display more button', 'masterchef' ),
					'toggle'			=> array(
						'true_toggle'	=> esc_html__( 'On', 'masterchef' ),
						'false_toggle'	=> esc_html__( 'Off', 'masterchef' ),
				...
				'more_button_text' => array(
					...
					'label'				=> esc_html__( 'More button text', 'masterchef' ),
				...
				'trim_words' => array(
					...
					'min_value'			=> 1,
					'step_value'		=> 1,
					'label'				=> esc_html__( 'Content words trimmed count', 'masterchef' ),
				),
				'slides_per_view' => array(
					...
					'min_value'			=> 1,
					'value'				=> 5,
					'label'				=> esc_html__( 'Number of slides per view', 'masterchef' ),
				),
				'slides_per_group' => array(
					...
					'min_value'			=> 1,
					'value'				=> 1,
					'label'				=> esc_html__( 'Number slides per group', 'masterchef' ),
				),
				'slides_per_column' => array(
					...
					'min_value'			=> 1,
					'value'				=> 1,
					'label'				=> esc_html__( 'Multi Row Slides Layout', 'masterchef' ),
				),
				'space_between_slides' => array(
					...
					'min_value'			=> 0,
					'value'				=> 30,
					'label'				=> esc_html__( 'Width of the space between slides(px)', 'masterchef' ),
				),
				'duration_speed' => array(
					...
					'min_value'			=> 100,
					'value'				=> 500,
					'label'				=> esc_html__( 'Duration of transition between slides (ms)', 'masterchef' ),
				),
				'navigation' => array(
					'type'				=> 'switcher',
					'value'				=> 'true',
					'style'				=> ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'				=> esc_html__( 'Slider navigation', 'masterchef' ),
				),
				'pagination' => array(
					'type'				=> 'switcher',
					'value'				=> 'true',
					'style'				=> ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'				=> esc_html__( 'Slider pagination', 'masterchef' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			// Widgets area check.
			if ( 'sidebar' == $args['id'] || 'sidebar-secondary' == $args['id'] ) {
				$this->instance['slides_per_view']		= 1;
				$this->instance['slides_per_group']		= 1;
				$this->instance['slides_per_column']	= 1;
			}

			$footer_widget_columns = get_theme_mod( 'footer_widget_columns', masterchef_theme()->customizer->get_default( 'footer_widget_columns' ) );
			...
			$data_attr_line = 'class="masterchef-carousel swiper-container"';
			...
			if ( $posts_query ) {
				...
				echo '<h5>' . esc_html__( 'Posts not found', 'masterchef' ) . '</h5>';
			}
		...
		public function get_query_items( $query_args = array() ) {
			...
			$defaults_query_args = apply_filters( 'masterchef_carousel_default_query_args', array(
				'post_type'			=> 'post',
				'orderby'			=> 'date',
				'order'				=> 'DESC',
				'posts_per_page'	=> -1,
		...
		public function get_carousel_loop( $posts_query ) {
			...
			if ( ! $view_dir ) {
				return '<h5>' . esc_html__( 'View file not found', 'masterchef' ) . '</h5>';
			}
	...
	function tm_register_carosel_widgets() {
		register_widget( 'Masterchef_Carousel_Widget' );
	}
}