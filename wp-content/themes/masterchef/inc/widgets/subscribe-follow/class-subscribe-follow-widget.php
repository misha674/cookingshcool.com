...
class Masterchef_Subscribe_Follow_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__( 'Display subscribe form and follow links.', 'masterchef' );
		$this->widget_id          = 'masterchef_widget_subscribe_follow';
		$this->widget_name        = esc_html__( 'Subscribe and Follow', 'masterchef' );
		$this->settings           = array(
			'enable_subscribe' => array(
				...
				'options' => array(
					'enable_subscribe' => esc_html__( 'Enable Subscribe Box', 'masterchef' ),
				),
			),
			'subscribe_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'masterchef' ),
				'label' => esc_html__( 'Subscribe Title', 'masterchef' ),
			),
			'subscribe_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Subscribe text message', 'masterchef' ),
			),
			'subscribe_input' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Enter your email', 'masterchef' ),
				'label' => esc_html__( 'Subscribe input placeholder', 'masterchef' ),
			),
			'subscribe_submit' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'masterchef' ),
				'label' => esc_html__( 'Subscribe submit label', 'masterchef' ),
			),
			'subscribe_success' => array(
				'type'  => 'text',
				'value' => esc_html__( 'You successfully subscribed', 'masterchef' ),
				'label' => esc_html__( 'Subscribe success', 'masterchef' ),
			),
			'enable_follow' => array(
				'type'   => 'checkbox',
				'value'  => array(
				...
				'options' => array(
					'enable_follow' => esc_html__( 'Enable Follow Box', 'masterchef' ),
				),
			),
			'follow_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Follow', 'masterchef' ),
				'label' => esc_html__( 'Follow Title', 'masterchef' ),
			),
			'follow_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Follow text message', 'masterchef' ),
			),
			'enable_background' => array(
				...
				'options' => array(
					'enable_background' => array(
						'label' => esc_html__( 'Enable Custom Background', 'masterchef' ),
			...
			'background_color_subscribe' => array(
				'type'   => 'colorpicker',
				'label'  => esc_html__( 'Subscribe Background Color', 'masterchef' ),
				'master' => 'background_image',
			),
			'background_color_follow' => array(
				'type'   => 'colorpicker',
				'label'  => esc_html__( 'Follow Background Color', 'masterchef' ),
				'master' => 'background_image',
			),
		);

		add_action( 'wp_ajax_masterchef_subscribe', array( $this, 'process_subscribe' ) );
		add_action( 'wp_ajax_nopriv_masterchef_subscribe', array( $this, 'process_subscribe' ) );

		$this->errors = array(
			'nonce'     => esc_html__( 'Security validation failed', 'masterchef' ),
			'mail'      => esc_html__( 'Please, provide valid mail', 'masterchef' ),
			'mailchimp' => esc_html__( 'Please, set up MailChimp API key and List ID', 'masterchef' ),
			'internal'  => esc_html__( 'Internal error. Please, try again later', 'masterchef' ),
		);
	...
	public function widget( $args, $instance ) {
		...
		if ( $subscribe_enabled && $subscribe_template && $api_key && $list_id ) {
			...
			esc_html_e( 'Please set up MailChimp API key and List ID', 'masterchef' );
		}
	...
	public function get_social_nav() {
		return masterchef_get_social_list( 'widget' );
	}
	...
	public function get_subscribe_submit( $class ) {
		...
		$subscribe_submit = masterchef_render_icons( $subscribe_submit );
	...
	public function get_block_message( $block = 'follow' ) {
		...
		if ( ! empty( $message ) ) {
			return '<h5 class="' . $block . '-block__message">' . wp_kses( $message, wp_kses_allowed_html( 'post' ) ) . '</h5>';
		}
	...
	public function process_subscribe() {
		...
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'masterchef_subscribe' ) ) {
...
add_action( 'widgets_init', 'masterchef_register_subscribe_follow_widgets' );
function masterchef_register_subscribe_follow_widgets() {
	register_widget( 'Masterchef_Subscribe_Follow_Widget' );
}
